...
class TFRBFNet(object):
    '''
    其中RBF使用两种训练方式：
        - 使用梯度下降训练隐藏层到输出层的权重，类似BPNN
        - 直接使用Linear Regreesion, 求beta的先行方程解
    '''
    ...
    def setup(self, ni, nh, no):
        ...
        self.input_layer = tf.placeholder(tf.float32,[None, self._input_n], name = 'input_layer')
        self.output_layer = tf.placeholder(tf.float32,[None, self._output_n], name = 'output_layer')
    ...
    def LinearTrain(self):
        '''直接使用公式求解隐藏层到输出层的beta参数，其中涉及到求逆操作'''
    ...
    def gradientTrain(self):
        ...
        for i in range(self.max_iter):
            ...
            if i%100 == 0:
                print('iter: %d '%(i))
                print('loss :',self.sess.run(self.loss,feed_dict={self.input_layer:self._inputs,self.output_layer:self._outputs}))
    ...
    def addLayer(self, inputs ,inputs_size, output_size,activationFun=None):
        '''
        添加隐藏层->输出层
        只有一层,参数较少，没有什么必要加正则以及dropout
        '''
